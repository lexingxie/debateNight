...
\TODO{LX}{Need a definition of ``the set of all diffusion scenarios (aka valid graphs) for a given set of nodes'', this is needed in the next section. Suggestion this to be $\Upsilon(v_{1:k})$.}
...
since the set of tweets $v_1:k$ are ordered in time,
and each retweet $v_i,~i=2,\ldots,k$ is triggered by one and only one parent among $v_{1:i-1}$.
With a slight abuse of notation,
we use $G^{k}$ to denote {\em one} of the valid diffusion trees
containing the first k tweets $v_1:k$, and $G^{k+1} = G^i \bigcup (v_i, v_{k+1})$, $i\in {1,\ldots,k}$
to denote {\em one} of the valid diffusion trees of $k+1$ tweets, in which the last tweet $v_{k+1}$ is triggered by
the i$^{th}$ tweet $v_i$.
\begin{equation} \label{eq:prob-scenario}
	\mathds{P}(G) = \prod_{(v_a,v_b) \in E} \mathds{P}\big((v_a, v_b)\big)
\end{equation}
...
\TODO{LX}{The notation between the two pieces of red texts here is very confusing: $z(v_i, v_k)$ is already declared to be part of $G$, and $z(v_i, v_k)$ is an (ordered) set not probability. What is $z(v_i,v_k | G)$ then?}
\verify{\hl{Suggestion: Eq(4) has a probablistic interpretation, just keep that, and remove this section.}}
...
\TODO{LX}{the two letter notation $VG$ should have been defined in the last section. See the suggested def of $\Upsilon(v_{1:k})$.}
\verify{
\begin{align}
